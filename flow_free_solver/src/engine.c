...
int game_num_free_coords_dead(const game_info_t* info,
                         const game_state_t* state,
                         int x, int y) {
	...
	for (int dir=0; dir<4; ++dir) {
		pos_t neighbor_pos = offset_pos(info, x, y, dir);
		if (neighbor_pos != INVALID_POS &&
		    state->cells[neighbor_pos] != TYPE_PATH) {
			++num_free;
		}
	}
...
int game_num_free_pos_dead(const game_info_t* info,
                      const game_state_t* state,
                      pos_t pos) {
	...
	return game_num_free_coords_dead(info, state, x, y);
