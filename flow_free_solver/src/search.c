...
int game_dijkstra_search(const game_info_t* info,
	...
	tree_node_t* root = node_create(NULL, info, init_state, ROOT);
	...
	while (!heapq_empty(&pq)){
		...
		int colour = game_next_move_color(info, &(current->state));
		...
		for (int direction = DIR_LEFT; direction <= DIR_DOWN; direction++){
			...
			if (game_can_move(info, &(current->state), colour, direction)){

				//Create child node
				tree_node_t* child = node_create(current, info, &(current->state), direction);
				...
				game_make_move(info, &(child->state), colour, direction);
